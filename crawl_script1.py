...
logger.log("Starting crawl (PID:%d CPUs:%d/%d VPN:%s sites:%d time0:%s CWD:%s)\n\n command: %s" % (
...
for vcc in VANTAGES:
    # try:
    vpn.connect(vcc)
    # TODO: error handling if above fails -- if country code wrong for instance
    # except:
    #    logger.log("VPN connection to %s FAILED" % (vcc,))
    #    continue
    ...
    for i, site in enumerate(sites):
        ...
        # - LATER test implicit consent by browsing (random) links on page: randomly_browse=True
        cs = engine.openwpm_crawl_sequence(site)
        assert VPNutil.is_route0_via_tun()
        manager.execute_command_sequence(cs)
        logger.log("Visit #%d: %s (from:%s @%.fs)" % (i + 1, site, vcc, time()-TIME0))
